...
namespace Toy_Synthesizer.Game.CommonUtils.RawValueStorage
{
    ...
    // Provides 8 bytes of raw value storage for any unmanaged type.
    ...
    public unsafe struct RawValueStorage_8B : IRawValueStorage
    {
        public const int STORAGE_SIZE = 8;

        static int IRawValueStorage.StorageSize => STORAGE_SIZE;

        public static RawValueStorage_8B From<T>(T value) where T : unmanaged
        {
            return ValueStorageUtils.From<T, RawValueStorage_8B>(value);
        }
        ...
        {
            return ValueStorageUtils.Read<T, RawValueStorage_8B>(ref this);
        }
