...
namespace Toy_Synthesizer.Game.CommonUtils.RawValueStorage
{
    ...
    // Provides 32 bytes of raw value storage for any unmanaged type.
    ...
    public unsafe struct RawValueStorage_32B : IRawValueStorage
    {
        public const int STORAGE_SIZE = 32;

        static int IRawValueStorage.StorageSize => STORAGE_SIZE;

        public static RawValueStorage_32B From<T>(T value) where T : unmanaged
        {
            return ValueStorageUtils.From<T, RawValueStorage_32B>(value);
        }
        ...
        {
            return ValueStorageUtils.Read<T, RawValueStorage_32B>(ref this);
        }
